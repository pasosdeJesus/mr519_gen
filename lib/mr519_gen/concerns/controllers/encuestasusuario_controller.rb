    ...
    module Controllers
      module EncuestasusuarioController
        ...
        included do
          ...
          def filtrar_ca(reg)
            reg = reg.where(usuario_id: current_usuario.id) if cannot?(:manage, Mr519Gen::Encuestausuario)
            reg
          end
          ...
          def index_reordenar(c)
            c.reorder("mr519_gen_encuestausuario.fecha DESC")
          end
          ...
          def creartodousuario
            ...
            redirect_to(
              encuestausuario_path(e),
              notice: "Creadas encuestas para #{numu} usuarios",
            )
          end

          def resultados_antes_presentar(formulario_id); end
